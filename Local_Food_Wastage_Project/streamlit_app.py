...
DB_PATH = "Local_Food_Wastage_Project/local_food.db"   # SQLite DB file in repo root
...
# ------------------ BROWSE & FILTER ------------------
    ...
    providers = run_query_df("SELECT DISTINCT Name FROM providers ORDER BY Name")["Name"].dropna().tolist()
    ...
    base_sql = '''
        SELECT 
            f.Food_ID, UPPER(SUBSTR(f.Food_Name,1,1)) || LOWER(SUBSTR(f.Food_Name,2)) AS Food_Name, 
...
# ------------------ QUERIES ------------------
    ...
    if choice == "Providers and Receivers by City":
        ...
        if city:
            query = """
                SELECT Name as provider_name, Type as provider_type, Address, City, Contact
                FROM providers
                WHERE City = ?
            """
        ...
        query = """
            ...
            FROM food_listings f
            JOIN providers p ON f.Provider_ID = p.Provider_ID
            GROUP BY p.City ORDER BY total_listings DESC
        """
        st.dataframe(run_query_df(query))
        ...
        query = """
            ...
            FROM food_listings
            GROUP BY Food_Type ORDER BY total_listings DESC
        """
        st.dataframe(run_query_df(query))
...
# ------------------ CONTACTS ------------------
    ...
    with tab1:
        dfp = run_query_df("SELECT Provider_ID, Name, Type, City, Contact FROM providers ORDER BY Name")
    ...
    with tab2:
        dfr = run_query_df("SELECT Receiver_ID, Name, Type, City, Contact FROM receivers ORDER BY Name")
...
# ------------------ CRUD ------------------
    ...
    if crud_tab == "providers":
        ...
        if submitted:
            run_exec("INSERT INTO food_listings (Food_Name,Quantity,Expiry_Date,Provider_ID,Provider_Type,Location,Food_Type,Meal_Type) VALUES (?,?,?,?,?,?,?,?)",
    ...
    if st.button("Delete Row"):
        id_col = {"providers":"Provider_ID","receivers":"Receiver_ID","food_listings":"Food_ID","claims":"Claim_ID"}[crud_tab]
        run_exec(f"DELETE FROM {crud_tab} WHERE {id_col}=?", (del_id,))
